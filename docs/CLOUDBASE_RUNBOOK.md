...
- 自动创建示例服务 `flask-3xwi`、HTTPS 域名和 MySQL；数据库 root 密码保持遮挡，未复制、显示或写入仓库。
...
- `DATABASE_URL=mysql+pymysql://...`（密码 URL 编码）
...
3. 为云托管服务开启连接数据库所在 VPC 的私有网络，只使用数据库内网地址；外网直连只允许临时调试且用后立即关闭。
