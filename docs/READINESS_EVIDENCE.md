...
核对日期：2026-09-13。状态依据当前提交和负责人提供的后台截图；上线准备分支已推送 GitHub，尚未上传微信业务代码或提交审核。
...
| 小程序构建与包体 | 本地完成 | 微信开发者工具编译成功；当前估算主包约360 KiB；自动包体及被排除资源引用门禁 | 体验版真机 iPhone/Android 回归 |
| 发布权限 | GitHub 准备分支已授权并完成；微信操作未授权 | `codex/release-readiness` 已同步；微信上传脚本默认拒绝执行 | 负责人逐次确认微信上传、审核和发布 |
